...
(function () {
  ...
  */
  ...
  /* ---------------- 点击音效（Web Audio 生成，柔和"叮咚"双音） ----------------
     复用单个 AudioContext，避免每次点击新建。
     使用两个和谐正弦音（C6 + E6）叠加，经低通滤波柔和化，快速指数衰减。
  */
  let clickCtx = null;
  ...
  function getClickCtx() {
    ...
    if (!clickCtx) {
      ...
      clickFilter.connect(clickCtx.destination);
    }
  ...
  function playClickSound() {
    try {
      ...
      master.connect(clickFilter);

      // 和谐双音：C6 (1046.5Hz) + E6 (1318.5Hz)
  ...
  function initClickSound() {
    if (reduceMotion) return;
    const sel = 'button, a, input, label, .check-item, .theme-card, .results-block, .rank-card, .ledger-row, .music-toggle, [data-role="choose-theme"], [data-role="save"]';
    document.addEventListener('click', (e) => {
      // 忽略纯文本选择、禁用的提交、拖拽手柄等
      const t = e.target;
      if (t.closest('input[type="text"]') && document.activeElement === t) return;
      const interactive = t.closest(sel);
      if (!interactive) return;
